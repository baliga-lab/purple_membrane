...
;; Last Modified: March, 24, 2016 (migration to NetLogo 5.3.1)
;;
;; Other Contributors: Professor Nitin Baliga, Dr. Marc Facciotti, Dr. Kenia Whitehead,
;; Dan Gallagher, Paul Shannon, Wei-ju Wu
...
to setup-nodes
  ...
  ask nodes with [ who = 2 ] [ set name "bat"
                               ...
                               setxy (0.0 * max-pxcor) (0.6 * max-pycor)
                              ]

  ;; crtb1
  ask nodes with [ who = 3 ] [ set name "crtb1"
                               set nodetype "protein"
                               setxy (0.67 * max-pxcor) (0.5 * max-pycor) ]

  ;; brp
  ask nodes with [ who = 4 ] [ set name "brp"
                               set nodetype "protein"
                               setxy (0.48 * max-pxcor) (-0.45 * max-pycor) ]

  ;; bop
  ask nodes with [ who = 5 ] [ set name "bop"
                               set nodetype "protein"
                               setxy (-0.8 * max-pxcor) (0.0 * max-pycor)
                             ]
  ...
  ask nodes with [ who = 7 ] [ set name "phytoene"
                               set nodetype "metabolite"
                               setxy (0.8 * max-pxcor) (0.4 * max-pycor)
                               ]
  ...
  ask nodes with [ who = 9 ] [ set name "betaCarotene"
                               set nodetype "metabolite"
                               setxy (0.8 * max-pxcor) (-0.3 * max-pycor)
                               ]

  ;; retinal
  ask nodes with [ who = 10 ] [ set name "retinal"
                                set nodetype "metabolite"
                                setxy (0.3 * max-pxcor) (-0.6 * max-pycor)
                                ]
  ...
  ask nodes with [ who = 11] [ set name "bacterioRhodopsin"
                               ...
                               setxy (0.0 * max-pxcor) (-0.8 * max-pycor) ]
...
to connect-to [other-node edge-type]  ;; node procedure
  hatch-edges 1
    [ set label ""
      ...
      let new-out-edges lput self [out-edges] of from
      ask from [set out-edges new-out-edges]
      let new-in-edges lput self [in-edges] of into
      ask into [set in-edges new-in-edges]
...
to correct-links
      ;; bop - bacterioRhodopsin edge
     ask turtle 27 [set size 17]
     ask turtle 27 [set heading 135]
     ask turtle 27 [set ycor -7]
     ask turtle 27 [set xcor -6.5]

     ask turtle 28 [set heading 135]
     ask turtle 28 [set xcor -1.5]
     ask turtle 28 [set ycor -12]
     ask turtle 28 [set size 1.5]

     ;; retinal -> bacterioRhodopsin
     ask turtle 42 [set size 3]
     ask turtle 42 [set heading 234]
     ask turtle 42 [set ycor -12.1]
     ask turtle 42 [set xcor 2.3]
     ask turtle 43 [set heading 234]
     ask turtle 43 [set xcor 1.5]
     ask turtle 43 [set ycor -12.6]
     ask turtle 43 [set size 1.5]

     ;; bat -> brp
     ask turtle 18 [set size 17]
     ask turtle 18 [set heading 336]
     ask turtle 18 [set ycor 1]
     ask turtle 18 [set xcor 4.3]
     ask turtle 19 [set heading 147]
     ask turtle 19 [set xcor 7.5]
     ask turtle 19 [set ycor -6]
     ask turtle 19 [set size 1.5]

     ;; betaCarotene - > retinal
     ask turtle 39 [set size 9]
     ask turtle 39 [set heading 239]
     ask turtle 39 [set ycor -8]
     ask turtle 39 [set xcor 9]

     ask turtle 40 [set heading 230]
     ask turtle 40 [set xcor 6.5]
     ask turtle 40 [set ycor -9.4]
     ask turtle 40 [set size 1.5]

     ;; lycopene - > betaCarotene
     ask turtle 36 [set size 1.5]
     ask turtle 36 [set heading 180]
     ask turtle 36 [set ycor -2.5]
     ask turtle 36 [set xcor 13.6]

     ask turtle 37 [set heading 180]
     ask turtle 37 [set xcor 13.6]
     ask turtle 37 [set ycor -2.6]
     ask turtle 37 [set size 1.5]

     ;; phytoene -> lycopene
     ask turtle 33 [set size 4]
     ask turtle 33 [set heading 180]
     ask turtle 33 [set ycor 3.5]
     ask turtle 33 [set xcor 13.6]

     ask turtle 34 [set heading 180]
     ask turtle 34 [set xcor 13.6]
     ask turtle 34 [set ycor 2.0]
     ask turtle 34 [set size 1.5]

     ;; gernayl -> phytoene
     ask turtle 30 [set size 4]
     ask turtle 30 [set heading 180]
     ask turtle 30 [set ycor 10]
     ask turtle 30 [set xcor 13.6]

     ask turtle 31 [set heading 180]
     ask turtle 31 [set xcor 13.6]
     ask turtle 31 [set ycor 9]
     ask turtle 31 [set size 1.5]

     ;; crtb1

     ask turtle 3 [set heading 0]
     ask turtle 3 [set xcor 10]
     ask turtle 3 [set ycor 8.5]
     ask turtle 3 [set size 3]

     ;; bat -> bop
     ask turtle 21 [set size 13]
     ask turtle 21 [set heading 54]
     ask turtle 21 [set ycor 5]
     ask turtle 21 [set xcor -6.5]

     ask turtle 22 [set heading 234]
     ask turtle 22 [set xcor -12]
     ask turtle 22 [set ycor 1]
     ask turtle 22 [set size 1.5]


     ;; bat -> crtb1
     ask turtle 24 [set size 7]
     ask turtle 24 [set heading 100]
     ask turtle 24 [set ycor 9]
     ask turtle 24 [set xcor 5]

     ask turtle 25 [set heading 100]
     ask turtle 25 [set xcor 8]
     ask turtle 25 [set ycor 8.5]
     ask turtle 25 [set size 1.5]


     ;; light -> bat
     ask turtle 12 [set size 1]
     ask turtle 12 [set heading 220]
     ask turtle 12 [set ycor 11.7]
     ask turtle 12 [set xcor 1.2]

     ask turtle 13 [set heading 220]
     ask turtle 13 [set xcor 0.7]
     ask turtle 13 [set ycor 11.2]
     ask turtle 13 [set size 1.5]

     ;; oxygen -> bat
     ask turtle 15 [set size 1]
     ask turtle 15 [set heading 130]
     ask turtle 15 [set ycor 12]
     ask turtle 15 [set xcor -1.8]

     ask turtle 16 [set heading 130]
     ask turtle 16 [set xcor -1.2]
     ask turtle 16 [set ycor 11.4]
     ask turtle 16 [set size 1.5]
...
to print-description [ node ]
  clear-output
  output-print (word "Description: " ([name] of node))
